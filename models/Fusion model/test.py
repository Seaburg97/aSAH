...
def predict_mrs_outcome(model_path, age, hunt_hess_score, mfs_score, sebes_score, stacking_imaging_model):
    """
    ...
    Args:
        ...
        stacking_imaging_model: Stacking imaging model prediction (0-6 range)
    ...
    # Extracted standardization parameters for Stacking_imaging_model
    STACKING_MEAN = 1.4545661892529713
    STACKING_STD = 1.8101500388120393
    ...
    # Based on training results: ['Age', 'mFS_score', 'SEBES_score', 'Hunt-Hess_score']
    final_features = []
    lasso_selected_features = ['Age', 'mFS_score', 'SEBES_score', 'Hunt-Hess_score']
    ...
    # Step 5: Add the additional feature (Stacking_imaging_model)
    # This feature was added separately and needs manual standardization
    stacking_scaled = (stacking_imaging_model - STACKING_MEAN) / STACKING_STD
    final_features.append(stacking_scaled)
...
if __name__ == "__main__":
    ...
    print("  Stacking_imaging_model Mean: 1.4545661892529713")
    print("  Stacking_imaging_model Std:  1.8101500388120393")
    ...
    result = predict_mrs_outcome(
        ...
        stacking_imaging_model=2.94    # Stacking imaging model prediction
    )
    ...
    print(f"🔧 Usage:")
    print(f"   result = predict_mrs_outcome(model_path, age, hunt_hess, mfs, sebes, stacking)")
    ...
    for age, hunt_hess, mfs, sebes, stacking in test_cases:
        result = predict_mrs_outcome(model_path, age, hunt_hess, mfs, sebes, stacking)
        print(f"   Age {age}, H-H {hunt_hess}, mFS {mfs}, SEBES {sebes}, Stacking {stacking} → mRS: {result['mRS_prediction']}")